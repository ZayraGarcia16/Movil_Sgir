...
const numeroHabitaciones = Joi.string()
  ...
  .messages({
    "string.pattern.base": "El formato debe ser un número seguido de 'habitaciones' o 'habitación'.",
...
const createHotelSchema = Joi.object({
  ...
  precio,
  categoria,
}).unknown(false);  // Si deseas evitar campos no definidos
...
const updateHotelSchema = Joi.object({
  ...
  precio,
  categoria,
}).min(1);  // Se permite actualizar al menos un campo
