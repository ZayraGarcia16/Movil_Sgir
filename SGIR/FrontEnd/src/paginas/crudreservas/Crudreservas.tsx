...
const crudreservas = () => {
  ...
  const obtenerReservas = async () => {
    try {
      ...
      const response = await axios.get<Reserva[]>('http://192.168.0.11:9700/api/reservas');
  ...
  const agregarOActualizarReserva = async () => {
    ...
    try {
      if (reservaEdit) {
        await axios.put(`http://192.168.0.11:9700/api/reservas/${reservaEdit._id}`, nuevaReserva);
        setReservaEdit(null);  // Limpiar la edición
      } else {
        await axios.post('http://192.168.0.11:9700/api/reservas', nuevaReserva);
      }
  ...
  const eliminarReserva = async (id: string) => {
    try {
      await axios.delete(`http://192.168.0.11:9700/api/reservas/${id}`);
